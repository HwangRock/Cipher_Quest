...
## 프로젝트에서 고민한 사항
### 평문을 어떻게 관리할 것인가?
제한시간을 둔 서비스라서 버저비터같은 것을 생각해서 최대한 데이터를 읽을 때 latency를 없애고자 목표를 잡았습니다.<br>
그래서 유저의 스테이지 별로 평문을 저장하는 것보다는 redis에서 바로바로 평문을 읽는 것이 더 속도가 빠르기에 redis에 평문을 저장하는 식으로 결정했습니다.
...
여러가지 암호 알고리즘을 디자인패턴에 사용함으로써 다시금 알고리즘의 중요성과 코드 구조의 중요성을 깨달을 수 있었습니다. <br>
